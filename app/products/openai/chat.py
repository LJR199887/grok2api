...
async def _resolve_image(token: str, url: str, image_id: str) -> str:
    ...
    use_imgbed = is_imgbed_enabled() and fmt != "base64"

    # Formats that don't need downloading
    if fmt == "grok_url" and not use_imgbed:
        return url
    if fmt == "grok_md" and not use_imgbed:
        return f"![image]({url})"

    # Formats that require downloading, including ImgBed public-link mode.
    try:
    ...
    if use_imgbed:
        public_url = await upload_bytes_to_imgbed(
            filename_from_url(url, "image"),
            raw,
            mime,
        )
        if fmt in {"grok_md", "local_md"}:
...
async def completions(
    *,
    ...
    if adapter.image_urls:
        imgbed_enabled = is_imgbed_enabled()
